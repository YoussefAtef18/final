...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    await new Email(user, resetURL).sendPassordReset();
    res.status(200).json({
      status: 'success',
...
exports.contactUs = catchAsync(async (req, res, next) => {
  ...
  const contactURL = `${req.get('host')}/contact-us`;
  await new Email(user, contactURL, message).sendContactUS();
  res.status(200).json({
    status: 'success',
    message: 'your problem has been received',
  });
});
